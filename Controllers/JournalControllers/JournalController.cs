...
#pragma warning disable CS8602
        ...
        [HttpPut("{journalId}")]
        [Authorize]
        public async Task<ActionResult<Journals>> UpdateJournal(
            int journalId,
            [FromBody] JournalDTO journalDTO
        )
        {
            var userId = User.FindFirstValue(ClaimTypes.NameIdentifier);
            if (string.IsNullOrEmpty(userId))
            ...
            var journal = await dbContext.Journals.FirstOrDefaultAsync(j =>
                j.JournalId == journalId && j.UserId == userId
            );
